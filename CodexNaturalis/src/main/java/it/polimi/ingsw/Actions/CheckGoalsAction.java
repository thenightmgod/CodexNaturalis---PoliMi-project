...
public class CheckGoalsAction extends Actions{

    public CheckGoalsAction(VirtualView view, MainController manager, RMIServer server){
    ...
    public void executor() throws RemoteException {
        ...
        if(k != -1){
            GameController Grian = getManager().getControllersPerGame().get(k);
            Grian.checkGoals(getView().getName());
        }
    }
}
